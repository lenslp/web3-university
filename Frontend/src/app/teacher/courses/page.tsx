...
export default function MyCoursesPage() {
		...
		try {
			...
			for (const courseInfo of coursesData) {
				try {
					...
					const courseId: bigint =
						courseInfo?.id ?? courseInfo?.[0] ?? BigInt(0);
					const price: bigint =
						courseInfo?.price ?? courseInfo?.[2] ?? BigInt(0);
	...
	const handleEditSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			await updateCourse(
				editingCourse.id,
				priceStr || formatEther(editingCourse.price),
				courseData,
			);
		} catch (error: any) {
			try {
				await updateCourseRaw(
					editingCourse.id,
					priceStr || formatEther(editingCourse.price),
					JSON.stringify({
						title: editForm.title.trim() || editingCourse.title,
						description:
							editForm.description.trim() || editingCourse.description,
						duration: editForm.duration,
						updatedAt: new Date().toISOString().slice(0, 10),
					}),
				);
					...
					{!address ? (
						...
						<div className="bg-white/5 backdrop-blur-sm rounded-xl p-12 text-center border border-white/10">
							...
							<h3 className="text-2xl font-bold mb-2 text-white">
								{courses.length === 0 ? "还没有课程" : "未找到匹配的课程"}
							</h3>
